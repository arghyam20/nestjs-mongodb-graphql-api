...
export class CmsAdminService {
  ...
  async getAll(body: CmsListingInput): Promise<ApiResponse> {
    let getAllCmss = await this.cmsRepository.getAllPaginate(body);
  ...
  async update(body: UpdateCmsInput): Promise<ApiResponse> {
    ...
    let updateCms = await this.cmsRepository.updateById(updatedValue, body.id);
  ...
  async statusUpdate(body: UpdateCmsStatusInput): Promise<ApiResponse> {
    const updatedValue = {
      status: body.status,
    };

    let updateStatus = await this.cmsRepository.updateById(
      updatedValue,
      body.id
    );
