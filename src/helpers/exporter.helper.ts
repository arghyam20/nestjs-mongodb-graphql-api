...
export async function exportFile(fileName: string, fileType: string, data: any[]): Promise<{ statusCode: number, message: string }> {
    try {
        ...
        switch (fileType.toLowerCase()) {
            ...
            case 'word':
                const doc = new Document({ // Pass an empty options object
                    sections: [{
        ...
        return { statusCode: 500, message: `Error exporting file: ${error.message}` };
    }
}
